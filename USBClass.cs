/* PneuView 3 Test Utility v4
 * RMBIMedical  20 Jun 2026
...
namespace PV3TestUtility4
{
  ...
  public String blDeviceID = "Vid_04D8&Pid_003C";		// 0x04D8 is Microchip VID; 0x003C is the VID for the Microchip HID Bootloader, modified for use on the PV3 PCBA
  ...
  public int AttemptUSBConnection()
  {
    ...
    IntPtr pDeviceBroadcastHeader = Marshal.AllocHGlobal(Marshal.SizeOf(DeviceBroadcastHeader));
    ...
    IntPtr DeviceInfoTable = SetupDiGetClassDevs(ref InterfaceClassGuid, IntPtr.Zero, IntPtr.Zero, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    ...
    while (true)
    {
      ...
      IntPtr PropertyValueBuffer = Marshal.AllocHGlobal((int)dwRegSize);
      ...
      bool MatchFound = DeviceIDFromRegistry.Contains(DeviceIDToFind);
      if (MatchFound == true)
      {
  ...
  public int AttemptUSBConnectionToBootloader()
  {
    ...
    while (true)
    {
      ...
      if (SetupDiEnumDeviceInterfaces(DeviceInfoTable, IntPtr.Zero, ref InterfaceClassGuid, InterfaceIndex, ref InterfaceDataStructure))
      {
        ...
        {	
          // Did we reach the end of the list of matching devices in the DeviceInfoTable?
          // Could not find the device; must not have been attached.
          SetupDiDestroyDeviceInfoList(DeviceInfoTable);			// clean up the old structure we no longer need.
          return (CONNECTION_NOT_SUCCESSFUL);
        }
      }
      ...
      if (MatchFound == true)
      {
        ...
        // Need to call SetupDiGetDeviceInterfaceDetail() again, this time specifying a pointer to a SP_DEVICE_INTERFACE_DETAIL_DATA buffer with the correct size of RAM allocated.
        // First need to allocate the unmanaged buffer and get a pointer to it.
        IntPtr pUnmanagedDetailedInterfaceDataStructure = Marshal.AllocHGlobal((int)StructureSize);
        DetailedInterfaceDataStructure.cbSize = 6;	// Initialize the cbSize parameter (4 bytes for DWORD + 2 bytes for unicode null terminator)
        Marshal.StructureToPtr(DetailedInterfaceDataStructure, pUnmanagedDetailedInterfaceDataStructure, false); //Copy managed structure contents into the unmanaged memory buffer.
